...
def dummy_expression(i: int) -> float:
    return (i * i - 7 / 5) * 8 // 33 + 3.3
...
def test_runner(num_samples: int):
    ...
    for runner in [runner_loop, runner_thread, runner_process]:
        output_list_runner = runner(dummy_expression, input_list)
        assert output_list_runner == output_list
